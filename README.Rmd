...
library(tidyverse)
library(lubridate)
csvs <- dir("data", pattern="presslog.*csv", full.names = TRUE)

csv_df <- tibble(source = csvs) %>%
  mutate(data = source %>% purrr::map(.f = read_csv))

csv_df <- csv_df %>% tidyr::unnest(col=data)

# how many calls are duplicates?
doubles <- csv_df %>% select(-source) %>% duplicated() 
# remove the duplicates
csv_df <- csv_df %>% slice(which(!doubles)) 

csv_df %>% mutate(
  date = mdy_hm(`Call Received Date/Time`)
) %>%
  ggplot(aes(x = yday(date), fill=basename(source))) +
  geom_bar() + facet_grid(year(date)~.) +
  ggtitle("Number of reports by day and log")


csv_df %>% 
  mutate(
    `Closing Disposition or Cancel Code` = factor(`Closing Disposition or Cancel Code`),
    `Closing Disposition or Cancel Code` = reorder(`Closing Disposition or Cancel Code`, `Closing Disposition or Cancel Code`, length)
  ) %>%
  ggplot(aes(x = `Closing Disposition or Cancel Code`)) +
  geom_bar() +
  coord_flip() + 
  ggtitle("Number of reports by code")
...
We are using the `tabulizer` functionality to extract values from the pdf.
We specify the output to be a matrix using the method lattice (to reduce the number of possible return values).
